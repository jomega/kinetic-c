...
struct _DriveEntry {
    ...
    int cv;                   /* Cluster Version: Default 0 */
    int pins[K_DRIVE_SIZE];   /* Erase and Lock pin storage */
...
static bool  find_drive_by_id(struct timeval t, const char *id);
static char *trim_json_string(char *str);
static int   getDriveState(DriveEntries *);
void         print_drive_entry(DriveEntries *drv);
...
static int getDriveState(DriveEntries *drv) {
    ...
    if(client != NULL) {
        ...
        strncpy(sessionConfig.host, drv->interfaces[0].ip4, HOST_NAME_MAX);

        sessionConfig.clusterVersion = drv->cv; // TODO: How do we determine clusterversion?
        sessionConfig.identity       = 1;
        sessionConfig.useSsl         = (drv->tls_port > 0) ? true : false;
        sessionConfig.port           = (sessionConfig.useSsl) ? drv->tls_port : drv->mcast_port;
...
static bool find_drive_by_id(struct timeval t, const char *id) {
    ...
    for(i = 0; i < driveList->used; i++) {
        ...
        if(strstr((driveList->entries[i])->wwname, id)) {
            ...
            int s = getDriveState(driveList->entries[i]);

            if(s != (driveList->entries[i])->state) {
                (driveList->entries[i])->state = s;
                print_drive_entry(driveList->entries[i]);
            }
...
static int discover_service(char *host, int port) {
    ...
    for (;;) {
        ...
        if(json_object_object_get_ex(obj, "world_wide_name", &val)) {
            ...
            if(!find_drive_by_id(arrival_time, s_ptr)) {
                ...
                new_entry->state = getDriveState(new_entry);

                driveList->entries[driveList->used] = new_entry;
                driveList->used++;

                print_drive_entry(new_entry);
            }
